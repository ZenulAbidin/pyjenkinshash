# This implementation is adapded from https://stackoverflow.com/questions/3279615/python-implementation-of-jenkins-hash
# - Ali
...
def lookup3_mix(a, b, c):
    a &= 0xffffffff; b &= 0xffffffff; c &= 0xffffffff
    a -= c; a &= 0xffffffff; a ^= lookup3_rot(c,4);  a &= 0xffffffff; c += b; c &= 0xffffffff
    b -= a; b &= 0xffffffff; b ^= lookup3_rot(a,6);  b &= 0xffffffff; a += c; a &= 0xffffffff
    c -= b; c &= 0xffffffff; c ^= lookup3_rot(b,8);  c &= 0xffffffff; b += a; b &= 0xffffffff
    a -= c; a &= 0xffffffff; a ^= lookup3_rot(c,16); a &= 0xffffffff; c += b; c &= 0xffffffff
    b -= a; b &= 0xffffffff; b ^= lookup3_rot(a,19); b &= 0xffffffff; a += c; a &= 0xffffffff
    c -= b; c &= 0xffffffff; c ^= lookup3_rot(b,4);  c &= 0xffffffff; b += a; b &= 0xffffffff
    return a, b, c

def lookup3_final(a, b, c):
    a &= 0xffffffff; b &= 0xffffffff; c &= 0xffffffff
    c ^= b; c &= 0xffffffff; c -= lookup3_rot(b,14); c &= 0xffffffff
    a ^= c; a &= 0xffffffff; a -= lookup3_rot(c,11); a &= 0xffffffff
    b ^= a; b &= 0xffffffff; b -= lookup3_rot(a,25); b &= 0xffffffff
    c ^= b; c &= 0xffffffff; c -= lookup3_rot(b,16); c &= 0xffffffff
    a ^= c; a &= 0xffffffff; a -= lookup3_rot(c,4);  a &= 0xffffffff
    b ^= a; b &= 0xffffffff; b -= lookup3_rot(a,14); b &= 0xffffffff
    c ^= b; c &= 0xffffffff; c -= lookup3_rot(b,24); c &= 0xffffffff
    return a, b, c

def jenkins_hash_lookup3_double(data, initval = 0, initval2 = 0):
    ...
    a, b, c = lookup3_final(a, b, c)

    return c, b

def jenkins_hash_lookup3(data, initval=0):
    c, b = jenkins_hash_lookup3_double(data, initval, 0)
    return c